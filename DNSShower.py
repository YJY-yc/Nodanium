...
# ===== 标准 Linux hosts 默认模板 =====
def generate_linux_default_hosts(hostname: str = "") -> str:
    """生成标准 Linux 默认 hosts 文件内容（含 localhost、主机名、IPv6 条目）"""
    if not hostname:
        try:
            hostname = socket.gethostname()
        except Exception:
            hostname = "localhost"
    hostname = hostname.split(".")[0].strip() or "localhost"
    return (
        "# Hosts file generated by Nodanium\n"
        "# /etc/hosts - local name resolution database\n"
        "\n"
        "127.0.0.1\tlocalhost\n"
...
def write_hosts_with_privileges(content: str) -> None:
    ...
    for tool in ("pkexec", "sudo", "su"):
        if tool == "pkexec":
            ...
            candidate = ["sudo", "sh", "-c", "cat '%s' > '%s' && rm -f '%s'" % (tmp_file, HOSTS_PATH, tmp_file)]
        else:
            candidate = ["su", "-c", "cat '%s' > '%s' && rm -f '%s'" % (tmp_file, HOSTS_PATH, tmp_file)]
        try:
...
def init_dns_tab(p):
    ...
    def on_init_dns(event):
        """初始化/重置DNS按钮处理函数：在 Linux 上写标准默认 hosts 模板，
        自动检测主机名、自动提权，并保证有明确结果反馈。"""
        if sys_type != "Windows":
            msg = (
                "确定要重置 hosts 文件吗？\n\n"
                "将把 /etc/hosts 重置为标准 Linux 默认模板\n"
                "（自动检测主机名，含 127.0.0.1 localhost、"
                "127.0.1.1 <主机名>、::1 ip6-localhost 等），\n"
                "可能使其他软件添加的 DNS 记录失效。\n"
            )
        else:
            msg = (
                "确定要重置 hosts 文件吗？\n\n"
                "将覆盖当前 hosts 文件内容，可能导致部分网站无法访问。"
            )
        confirm_dlg = wx.MessageDialog(panel, msg, "", wx.YES_NO | wx.ICON_WARNING)
        confirm_dlg.SetYesNoLabels("我知道我在做什么", "取消")
        if confirm_dlg.ShowModal() != wx.ID_YES:
            return
        try:
            if sys_type == "Windows":
                ...
                content = generate_linux_default_hosts()

            write_hosts_with_privileges(content)
            flush_dns_cache()

            wx.MessageBox("hosts 文件已重置成功", "成功", wx.OK | wx.ICON_INFORMATION)
            refresh_dns_list()
            ...
            wx.MessageBox("未找到 host.py 文件或 host 变量", "错误", wx.OK | wx.ICON_ERROR)
        except Exception as e:
